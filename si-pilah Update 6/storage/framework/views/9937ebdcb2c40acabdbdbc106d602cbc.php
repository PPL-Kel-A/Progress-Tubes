...
<?php $__env->startSection('page-description', 'Kelola pesan dan pertanyaan dari pengguna'); ?>
...
<div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
    <div class="flex items-center gap-2">
        <a href="<?php echo e(route('admin.contact.messages')); ?>" class="px-4 py-2 rounded-xl text-sm font-semibold transition <?php echo e(request('status') === null ? 'bg-sipilah-700 text-white shadow-sm' : 'bg-gray-50 text-gray-600 hover:bg-gray-100'); ?>">
            Semua
        </a>
        <a href="<?php echo e(route('admin.contact.messages', ['status' => 'Baru', 'search' => request('search')])); ?>" class="px-4 py-2 rounded-xl text-sm font-semibold transition <?php echo e(request('status') === 'Baru' ? 'bg-sipilah-700 text-white shadow-sm' : 'bg-gray-50 text-gray-600 hover:bg-gray-100'); ?>">
            Baru <?php if($unreadCount > 0): ?><span class="ml-1 bg-red-500 text-white text-[10px] px-1.5 py-0.5 rounded-full"><?php echo e($unreadCount); ?></span><?php endif; ?>
        </a>
        <a href="<?php echo e(route('admin.contact.messages', ['status' => 'Dibaca', 'search' => request('search')])); ?>" class="px-4 py-2 rounded-xl text-sm font-semibold transition <?php echo e(request('status') === 'Dibaca' ? 'bg-sipilah-700 text-white shadow-sm' : 'bg-gray-50 text-gray-600 hover:bg-gray-100'); ?>">
            Dibaca
        </a>
        <a href="<?php echo e(route('admin.contact.messages', ['status' => 'Dibalas', 'search' => request('search')])); ?>" class="px-4 py-2 rounded-xl text-sm font-semibold transition <?php echo e(request('status') === 'Dibalas' ? 'bg-sipilah-700 text-white shadow-sm' : 'bg-gray-50 text-gray-600 hover:bg-gray-100'); ?>">
            Dibalas
        </a>
    </div>

    <form method="GET" action="<?php echo e(route('admin.contact.messages')); ?>" class="flex">
        <?php if(request('status')): ?>
            <input type="hidden" name="status" value="<?php echo e(request('status')); ?>">
        <?php endif; ?>
        <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Cari nama, email, atau subjek..." class="w-full md:w-64 border border-gray-200 rounded-l-xl px-4 py-2 text-sm focus:ring-2 focus:ring-sipilah-500 focus:border-transparent outline-none">
...
<div class="space-y-4">
    ...
    <div x-data="{ open: false, showReplyForm: false }" class="bg-white rounded-2xl shadow-sm border <?php echo e($message->status === 'Baru' ? 'border-l-4 border-l-green-500 border-gray-100' : 'border-gray-100'); ?> overflow-hidden">
        
        <div @click="open = !open; if(open && '<?php echo e($message->status); ?>' === 'Baru') { document.getElementById('read-form-<?php echo e($message->id); ?>').submit(); }" 
            ...
            <div class="flex items-start gap-4 flex-1">
                ...
                <div>
                    <div class="flex items-center gap-2 mb-1">
                        <h4 class="font-bold text-gray-800 <?php echo e($message->status === 'Baru' ? '' : 'text-gray-600'); ?>"><?php echo e($message->name); ?></h4>
                    ...
                    <p class="text-sm font-semibold text-gray-800 mb-0.5 <?php echo e($message->status === 'Baru' ? '' : 'text-gray-600'); ?>"><?php echo e($message->subject); ?></p>
                    <p class="text-xs text-gray-500 truncate max-w-xl"><?php echo e(Str::limit($message->message, 80)); ?></p>
                </div>
            </div>
            ...
            <div class="flex flex-col items-end gap-2 shrink-0 w-full md:w-auto">
                ...
                <span class="text-xs text-gray-400 font-medium"><?php echo e($message->created_at->diffForHumans()); ?></span>
            </div>
        ...
        <?php if($message->status === 'Baru'): ?>
            <form id="read-form-<?php echo e($message->id); ?>" action="<?php echo e(route('admin.contact.messages.read', $message)); ?>" method="POST" class="hidden">
                <?php echo csrf_field(); ?>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div class="md:col-span-1 space-y-4">
                        <div class="bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
                            ...
                            <div class="space-y-3">
                                <div>
                                    ...
                                    <p class="text-sm font-semibold text-gray-800"><?php echo e($message->name); ?></p>
                                </div>
                                <div>
                                    <p class="text-xs text-gray-500 mb-0.5">Email</p>
                                    <a href="mailto:<?php echo e($message->email); ?>" class="text-sm font-semibold text-sipilah-600 hover:underline"><?php echo e($message->email); ?></a>
                                </div>
                                <div>
                                    <p class="text-xs text-gray-500 mb-0.5">Tanggal</p>
                                    <p class="text-sm font-semibold text-gray-800"><?php echo e($message->created_at->translatedFormat('d F Y, H:i')); ?></p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex gap-2">
                            ...
                            <form action="<?php echo e(route('admin.contact.messages.delete', $message)); ?>" method="POST" onsubmit="return confirm('Yakin ingin menghapus pesan ini?');" class="flex-1">
                                <?php echo csrf_field(); ?>
                    ...
                    <div class="md:col-span-2 space-y-4">
                        <div class="bg-white p-5 rounded-xl border border-gray-100 shadow-sm relative">
                            ...
                            <h4 class="font-bold text-gray-800 mb-3"><?php echo e($message->subject); ?></h4>
                        ...
                        <?php if($message->status === 'Dibalas'): ?>
                            <div class="bg-blue-50/50 p-5 rounded-xl border border-blue-100 shadow-sm">
                                <div class="flex items-center justify-between mb-3">
                                    ...
                                    <span class="text-xs text-gray-400"><?php echo e($message->replied_at->translatedFormat('d F Y, H:i')); ?></span>
                                </div>
                                <div class="prose prose-sm max-w-none text-gray-700 whitespace-pre-line bg-white p-4 rounded-xl border border-blue-50">
                                    <?php echo e($message->admin_reply); ?>
                        ...
                        <div x-show="showReplyForm" x-collapse>
                            <form action="<?php echo e(route('admin.contact.messages.reply', $message)); ?>" method="POST" class="bg-white p-5 rounded-xl border border-sipilah-100 shadow-md">
                                <?php echo csrf_field(); ?>
                                ...
                                <?php else: ?>
                                    <div class="mb-3 flex items-center gap-2 text-xs text-orange-600 bg-orange-50 px-3 py-2 rounded-lg border border-orange-100">
                                        ...
                                        Pengirim bukan user terdaftar. Sistem akan mencatat balasan ini, namun Anda perlu membalas secara manual via email (<?php echo e($message->email); ?>).
                                    </div>
                                <?php endif; ?>
                                
                                <textarea name="admin_reply" rows="4" required placeholder="Tulis balasan di sini..." class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-sipilah-500 focus:border-transparent outline-none resize-none mb-3"><?php echo e($message->admin_reply); ?></textarea>
...
<?php echo $__env->make('admin.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\si-pilah Update 6\resources\views/admin/contact-messages.blade.php ENDPATH**/ ?>