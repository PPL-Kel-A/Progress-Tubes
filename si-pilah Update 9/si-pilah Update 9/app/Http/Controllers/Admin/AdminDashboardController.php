...
class AdminDashboardController extends Controller
{
    ...
    public function updateAbout(Request $request)
    {
        ...
        // Validasi: pastikan field tidak kosong berdasarkan section
        $rules = [];
        if ($section === 'hero') {
            $rules = [
            ...
            $rules = [
                ...
                'strategi_title' => 'required|string|max:255',
                'strategi_description' => 'required|string',
            ];
        } elseif ($section === 'sejarah') {
            for ($i = 1; $i <= 4; $i++) {
                $rules["sejarah_item_{$i}_year"] = 'required|string|max:255';
                $rules["sejarah_item_{$i}_title"] = 'required|string|max:255';
                $rules["sejarah_item_{$i}_desc"] = 'required|string';
            }
        } elseif ($section === 'team') {
            $rules['team_description'] = 'required|string';
            for ($i = 1; $i <= 7; $i++) {
                $rules["team_member_{$i}_name"] = 'required|string|max:255';
                $rules["team_member_{$i}_role"] = 'required|string|max:255';
            }
        } elseif ($section === 'layanan') {
            for ($i = 1; $i <= 4; $i++) {
                $rules["layanan_item_{$i}_title"] = 'required|string|max:255';
                $rules["layanan_item_{$i}_desc"] = 'required|string';
            }
        }
        ...
        if ($section === 'visi') {
            $visiFields = [
                'title' => $request->visi_title,
                'description' => $request->visi_description
            ];
            foreach ($visiFields as $key => $value) {
                AboutSetting::updateOrCreate(
                    ['section' => 'visi', 'key' => $key],
                    ['value' => $value]
                );
            }

            $strategiFields = [
                'title' => $request->strategi_title,
                'description' => $request->strategi_description
            ];
            foreach ($strategiFields as $key => $value) {
                AboutSetting::updateOrCreate(
                    ['section' => 'strategi', 'key' => $key],
                    ['value' => $value]
                );
            }
        } else {
    ...
    public function updateContact(Request $request)
    {
        ...
        // Validasi: pastikan field tidak kosong berdasarkan section
        $rules = [];
        if ($section === 'contact_hero') {
            $rules = [
                'contact_hero_title' => 'required|string|max:255',
                'contact_hero_description' => 'required|string',
            ];
        } elseif ($section === 'contact_info') {
            $rules = [
                'contact_info_email_1' => 'required|email|max:255',
                'contact_info_email_2' => 'required|email|max:255',
                'contact_info_phone' => 'required|string|max:255',
                'contact_info_phone_hours' => 'required|string|max:255',
                'contact_info_address_1' => 'required|string|max:255',
                'contact_info_address_2' => 'required|string|max:255',
            ];
        }
